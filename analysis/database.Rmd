...
```{r subset-tools}
tools.old <- tools %>%
    filter(Added < "2016-10-01") %>%
    mutate(Subset = "Old")
tools.new <- tools %>%
    filter(Added >= "2016-10-01") %>%
    mutate(Subset = "New")
tools.all <- mutate(tools, Subset = "All")

tools.combined <- bind_rows(tools.all, tools.old, tools.new)
```

```{r plot-categories-time}
cats.time.plot <- tools.combined %>%
    filter(Subset != "All") %>%
    group_by(Subset) %>%
    summarise_at(8:35, sum) %>%
    gather(key = Category, value = Count, -Subset) %>%
    mutate(Category = str_replace_all(Category, "([[:upper:]])", " \\1")) %>%
    mutate(Category = str_trim(Category)) %>%
    ...
    ggplot(aes(x = Category, y = Prop, colour = Subset)) +
    geom_point(size = 4) +
    scale_color_discrete(name = "Date added",
...
```{r plot-cats-tool}
cats.tools.time.plot <- tools.combined %>%
    filter(Subset != "All") %>%
    select(Name, Subset, 8:35) %>%
    gather(key = Category, value = TF, -Name, -Subset) %>%
    group_by(Name, Subset) %>%
    summarise(Cats = sum(TF)) %>%
    ungroup() %>%
    select(-Name) %>%
    group_by(Subset, Cats) %>%
    mutate(Count = n()) %>%
    unique() %>%
    group_by(Subset) %>%
    mutate(Prop = Count / sum(Count)) %>%
    ggplot(aes(x = factor(Cats, levels = 1:14), y = Prop, fill = Subset)) +
    geom_bar(stat = "identity", position = "dodge") +
    scale_fill_discrete(name = "Date added",
                         labels = c("After 2016-10-01", "Before 2016-10-01")) +
    scale_y_continuous(labels = scales::percent) +
