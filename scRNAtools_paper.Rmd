...
database can do so by submitting an issue to the project GitHub page
...
experiments, such as alignment to a reference genome or transcriptome and
quantification of expression, but is often extended to handle Unique Molecular
Identifiers (UMIs). Once an expression matrix has been obtained it is vital to
make sure the resulting data is of high enough quality. In the data cleaning
phase (Phase 2) quality control of cells is performed as well as filtering of
uninformative genes. Additional tasks may be performed to normalise the data or
impute missing values and exploratory analysis is often performed in this phase,
such as viewing the datasets in reduced dimensions to look for underlying
structure.
...
a valuable technique for devloping, testing and validating scRNA-seq tools. More
...
anlyses. As single-cell capture and sequencing technology continues to improve
