...
As single-cell RNA-sequencing datasets have become available the number of
analysis tools has dramatically increased. When new tools have become available
we have catalogued them in the scRNA-tools database (www.scRNA-tools.org). Tools
are categorised according to the analysis tasks they perform. Analysis of this
database shows us that many tools perform tasks specific to scRNA-seq analysis,
particularly clustering and ordering of cells. We find that the scRNA-seq
analysis community takes an open-source approach with most tools making their
code available under open-source licences and preprints being extensively used
as a means to describe methods. The scRNA-tools database provides a resource
...
outstripping Moore’s Law [Svensson 2017]. This new kind of transcriptomic data
...
challenges unique to the single-cell context. Specifically, scRNA-seq data is
extremely sparse (there is no expression measured for many genes in most cells),
contains large technical artefacts and the scientific questions of interest are
...
The bioinformatics community has embraced this new type of data with a plethora
of methods for the analysis of scRNA-seq data being produced. As such, keeping
up with the current state of scRNA-seq analysis is now a significant challenge
as the field is presented with a huge number of choices for approaching an
...
analysis applications their methods address but how they are published, licensed
and the platforms they use. Based on this database we gain insight into the
state of analysis tools in this rapidly developing field.
...
in order to find those most relevant to their needs. Anyone wishing to
contribute to the database can do so by submitting an issue to our Github page
...
attention in a paper describing their method and use. Of all the tools in
the database about half have been published in peer-reviewed journals and
another third are described in preprint articles, typically on the bioRxiv
preprint server. When the tools are divided by the date they were added to
the scRNA-tools database we can see that the majority of older tools have
been published while more recent tools are more likely to only be available as
preprints. This is a good demonstration of the delay imposed by the traditional
publication process. By publishing preprints and releasing software available
via repositories such as Github scRNA-seq tool developers make their methods
available to the community much earlier, allowing them to be used for analysis
and their methods improved prior to formal publication.
...
almost a fifth of tools. This is problematic as fellow developers must assume
...
are given in [TABLE], along with the associated analysis phases.
...
In this section we look at the categories that tools in the scRNA-tools database
...
Tasks for assigning cells (ordering and clustering) are the next most common.
...
analysis. Testing for differential expression (DE) is perhaps the most common
...
commonly performed by recent tools. The UMIs category has also seen a
...
change in cell ordering analysis which is the focus of a lower percentage 
of tools added after October 2016. The ordering of cells along a trajectory was
one of the first developments in scRNA-seq analysis and a decrease in the
development of these tools could indicate that researchers have moved on to
other techniques or that use has converged on a set of mature tools.

Looking at the tools that can perform at least one task in an analysis phase
we see similar trends. We see increases in the percentage of tools performing
tasks in Phase 1 (quantification), Phase 2 (quality control and filtering),
across multiple phases (visualisation and dimensionality reduction) and
alternative analysis tasks. In contrast the percentage of tools that perform
gene identification tasks (Phase 2) has decreased and the percentage assigning
cells (Phase 3) has remained steady. This too may indicate a maturation of the
analysis space as existing tools for performing standard scRNA-seq analyses are
deemed sufficient while there is still room for development in handling data
...
single analysis task, many perform at least two. We find that older tools tend
...
example alternative splicing, single nucleotide [?] variants and allele-specific
expression. Reconstruction of immune cell receptors is another area that has
received considerable attention. While tools that complete these tasks are
unlikely to ever dominate scRNA-seq analysis it is likely that we will see an
increase in methods for tackling alternative problems as researchers continue to
push the boundaries of what can be observed using scRNA-seq data.

# Discussion and conclusions

Over the last year we have seen the number of number of software tools for
analysing single-cell RNA-seq data double. As new tools have become available
we have catalogued them in the scRNA-tools database where we record the analysis
tasks that they can complete, along with additional information such as any
associated publications. Tool developers have focused their efforts on methods
for handling problems specific to scRNA-seq data, in particular clustering cells
into groups or ordering them along a trajectory. We have seen that the scRNA-seq
community is generally open and willing to share with methods being described
in preprint prior to publication and most software being released under
permissive open-source licenses.

The next few years promise to continue to produce significant developments
...
possible by scRNA-seq data. Some existing tools will continue to improve and
expand their functionality while others will cease to be updated and maintained.
As single-cell capture and sequencing technology continues to develop tools will
have to adapt to significantly larger datasets (in the millions of cells) which
may require specialised data structures and algorithms. Other types of single-
cell data will become available and projects such as the Human Cell Atlas will
provide comprehensive references, both of which will open up new avenues for
analysis.

As the field expands the scRNA-tools database will continue to be updated.
We hope that it provides a resource for researchers approaching an scRNA-seq
analysis as well as providing a record of the analysis landscape and how it
changes over time.
